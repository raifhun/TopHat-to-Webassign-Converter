...
def convert_file(filename):
    ...
    file_data = pd.read_excel(filename, sheet_name=2)  # the email address column isn't populated
    ...
    for i in range(0, len(usernames_fixed)):
        if file_data['participation_score'][i] == file_data['participation_weight'][i]:
            scores.append(1)
        else:
            scores.append(0)
